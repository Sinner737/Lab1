...
namespace lab_1
{
    ...
    {
        const string PathZapisiFailov = "ForRecords.txt";
        ...
        public void InitKatalogList()
        {
            ...
            for (int i = 0; i < FileAndDirectoryVersion.fileList.Count; i++)
            {
                ...
                // Добавление в список записи происходит в том случае, если не было найдено директории.
                if (flagZapisi == 1)
                    katalogList.Add(p);
            }
        }
        ...
        public void InitFileList()
        {
            ...
            while (!Reader.EndOfStream) // Начальное состояние.
            {
                ...
                string LinetSize = Reader.ReadLine();
                string LineCreated = Reader.ReadLine();
                string LineModified = Reader.ReadLine();
                string LineNote = Reader.ReadLine();
                string LineDirectory = Reader.ReadLine();

                p.Name = LineName;
                p.Size = LinetSize;
                p.Created = LineCreated;
                p.Modified = LineModified;
                p.Note = LineNote;
                p.Directory = LineDirectory;
